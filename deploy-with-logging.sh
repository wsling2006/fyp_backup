...
cd /var/www/fyp_system || { echo -e "${RED}Failed to navigate to project directory${NC}"; exit 1; }
...
pm2 restart fyp-backend || pm2 start npm --name "fyp-backend" -- run start:prod
...
echo "   pm2 logs fyp-backend | grep -E 'JwtStrategy|JwtAuthGuard|RolesGuard'"
echo ""
echo "3. Monitor logs in real-time:"
echo "   pm2 logs fyp-backend --raw"
echo ""
